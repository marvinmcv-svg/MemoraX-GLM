...
import { createTigoPayment } from '@/lib/payments/tigo'
import type { PlanId, BillingCycle } from '@/lib/payments'

export const dynamic = 'force-dynamic'

// POST /api/payments/tigo  body: { userId, plan, cycle }
export async function POST(req: NextRequest) {
  try {
    ...
    const tigo = await createTigoPayment({ userId, plan, cycle })
    return NextResponse.json(tigo)
  } catch (e: any) {
    console.error('[payments/tigo] error', e)
